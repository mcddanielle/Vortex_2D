...
ax.set_xlim(0.5, 4.0)
#add a sub-subplot
rect = [0.4,0.5,0.4,0.5]  #x0,y0,x_size,y_size (percent)
...
##################
rect = [0.75,0.55,0.3,0.4]  #x0,y0,x_size,y_size (percent)
...
ax = subplot(G[3:5,2:4])
ax.set_aspect('equal')
cset = ax.contourf(X, Y, Z, zdir='z', offset=-1, cmap=cm.hot)
#ax.set_ticks('off') # clear x- and y-axes
...
ax2 = subplot(G[2,2:4])
ax2.plot(prof_xbin,prof_x)

ax3 = subplot(G[3:5,-1])
ax3.plot(prof_y,prof_xbin)
