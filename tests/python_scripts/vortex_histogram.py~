...
#ax = plt.contour(X, Y, Z, 15, linewidths = 0.5, colors = 'k')
#ax = plt.pcolormesh(X, Y, Z, cmap = plt.get_cmap('rainbow'))

ax.plot_surface(X,Y,Z,rstride=1, cstride=1, alpha=1.0, cmap='hot',linewidth=0)
...
ax.set_ylabel('Y')
ax.set_ylim(0, 36.5)
ax.set_zlabel('Vortex Amplitude')
ax.set_zlim(0, 100)
'''
...
#
################
# Third subplot - color map of vortex stamp
#################
...
#insert s(k) as a subplot
rect = [0.25,0.25,0.75,0.75]  #x0,y0,x_size,y_size (percent)
ax2 = add_subplot_axes(ax,rect)
ax2.imshow(image)
